...
# Scenario y modifications
output_excel = "data/y_scenario.xlsx"
with pd.ExcelWriter(output_excel) as writer:
    
    # ===== Scenario 1: Consumption structure change =========================
    col_map = {"RCC": "Rural resident consumption", "UCC": "Urban resident consumption", "GCC": "Government consumption"}
    
    # Redistribute 10% of capital formation to 3 consumption sectors
    for sc, cname in col_map.items():
        y_new, Wf_new = y_df.copy(), Wf_df.copy()
        delta_y = 0.1 * y_df["Fixed capital formation"].sum()
        old_y_sum = y_df[cname].sum()
        scale_y = (old_y_sum + delta_y) / old_y_sum
        y_new["Fixed capital formation"] *= 0.9
        y_new[cname] *= scale_y
        Wf_new[cname] *= scale_y
        y_new.to_excel(writer, sheet_name=f"y_{sc}")
        Wf_new.to_excel(writer, sheet_name=f"Wf_{sc}")
        
    # ===== Scenario 2: Consumption behavior change ==========================
    y_new, Wf_new = y_df.copy(), Wf_df.copy()
    
    # Reduce household food expenditure and food waste by 10%
    y_new.iloc[5, [0, 1]] *= 0.9
    Wf_new.iloc[13, [0, 1]] *= 0.9
    y_new.to_excel(writer, sheet_name="y_sc2")
    Wf_new.to_excel(writer, sheet_name="Wf_sc2")

    # ===== Scenario 3: Industrial structure change ==========================
    y_new, Wf_new = y_df.copy(), Wf_df.copy()
    
    # Increase tertiary industry demand, overall waste generation increases
    y_new.iloc[28:42, :] *= 1.1
    y_old_sum = y_df.select_dtypes(include=[np.number]).sum().sum()
    y_new_sum = y_new.select_dtypes(include=[np.number]).sum().sum()
    scale_y_total = y_new_sum / y_old_sum
    Wf_new *= scale_y_total
    y_new.to_excel(writer, sheet_name="y_sc3")
    Wf_new.to_excel(writer, sheet_name="Wf_sc3")
...
scenarios = ['GCC', 'RCC', 'UCC']
base_path = "result/result_net_all.xlsx"

for scenario in scenarios:
    print(f"\n=== Processing {scenario} scenario ===")
    
    # Load scenario data
    y_df = pd.read_excel(f'data/y_scenario.xlsx', sheet_name=f"y_{scenario}", index_col=[0])
    Wf_df = pd.read_excel(f'data/y_scenario.xlsx', sheet_name=f"Wf_{scenario}", index_col=[0,1])
    ...
    # Save detailed results by demand sector
    output_excel = f"result/scenarios/{scenario}/result_p.xlsx"
    with pd.ExcelWriter(output_excel) as writer:
    ...
    # Aggregate post-consumption waste footprint
    ...
    for i, dname in enumerate(p_labels[:2]):  # Only first two
        ...
        x_ii_m = np.dot(np.dot(B_II_II, S), diag_y_ii)  # (4 x 27)
        df_xii_m = pd.DataFrame(x_ii_m, index=waste_columns, columns=m_labels)
        w_m_sum[dname] = df_xii_m.sum(axis=0)  # Sum over treatment types
    
    # Save aggregated results
    with pd.ExcelWriter(f"result/scenarios/{scenario}/result.xlsx") as writer:
    ...
    def compare_excel_files(file1_path, file2_path, output_path):
        with pd.ExcelWriter(output_path) as writer:
            sheets = pd.ExcelFile(file1_path).sheet_names
            for sheet in sheets:
                ...
                diff_df.to_excel(writer, sheet_name=sheet, index=False)
    
    compare_excel_files(
        f"result/scenarios/{scenario}/result.xlsx",
        base_path,
        f"result/scenarios/{scenario}/result_diff.xlsx"
    )
